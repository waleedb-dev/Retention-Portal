  ...
  const handleShortFormSubmit = async () => {
    ...
    if (deal.dealId) {
      try {
        ...
        if (assignmentError) {
          console.error("[fixed-payment] Error checking assignment:", assignmentError);
          toast({ 
            title: "Error", 
        ...
        if (!assignment) {
          console.warn("[fixed-payment] Unauthorized submission attempt - deal not assigned to agent", {
            dealId: deal.dealId,
        ...
        console.error("[fixed-payment] Error during authorization check:", authError);
        toast({ 
          title: "Error", 
    ...
    if (!submissionId && deal.dealId) {
      try {
        ...
        console.warn("[fixed-payment] Error fetching monday_item_id:", fetchError);
      }
    }
    
    if (!submissionId) {
      console.error("[fixed-payment] Missing submission_id:", {
    ...
    console.log("[fixed-payment] Using submission_id:", submissionId);
    ...
    try {
      ...
      if (submissionId) {
        try {
          ...
          console.warn("[fixed-payment] Error fetching lead_vendor:", fetchError);
        }
      }
      ...
      // Get draft date from form first, then raw deal data
      const finalDraftDate = draftDate || (rawDeal && typeof rawDeal.draft_date === "string" ? rawDeal.draft_date : null);
      ...
      {step === "instructions" ? (
        <div className="space-y-4 p-4 border rounded-lg bg-card">
          ...
          {instructions.kind === "call" ? (
            <div className="rounded-md border bg-blue-50 dark:bg-blue-950/30 p-4 space-y-4">
              <div className="text-sm font-medium text-blue-900 dark:text-blue-100">Call Instructions</div>
              <div className="text-sm text-blue-800 dark:text-blue-200">
                Call <span className="font-semibold text-blue-900 dark:text-blue-100">{instructions.phone}</span>
              </div>
              {instructions.optionsInstruction ? (
                <div className="text-sm text-blue-800 dark:text-blue-200">{instructions.optionsInstruction}</div>
              ) : null}
              <div>
                <div className="text-sm text-blue-800 dark:text-blue-200">When connected, say:</div>
                <div className="mt-2 rounded-md border border-blue-200 dark:border-blue-800 bg-white dark:bg-blue-950/50 p-3 text-sm text-blue-900 dark:text-blue-100">
                  &quot;{instructions.script}&quot;
                </div>
              </div>
            </div>
