...
export default async function handler(req: NextApiRequest, res: NextApiResponse<VicidialRouteResponse>) {
  ...
  const body = (req.body ?? {}) as DtmfRequestBody;
  const fn = body.vicidial_function ?? "send_dtmf";
  ...
  if (!body.value) {
    return res.status(400).json({
      ok: false,
      error: "value is required for send_dtmf",
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: "VICIdial send_dtmf request failed",
